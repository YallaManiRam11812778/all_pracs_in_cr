...
for pdf in pdfs:
    ...
    invoice_details = tabula.read_pdf(pdf, area=[[A,B,A+D,B+C]], pandas_options={'header':None},pages='1')
    ...
    guest_address = tabula.read_pdf(pdf, area=[[A,B,A+D,B+C]], pandas_options={'header':None},pages='1')
    print("\n\n",guest_address,"\n\n")
    """ Take Units if in pixels (px/96 dpi) * 72 """
    Top = A = 218.25
    Left = B = 11.25
    ...
    charges_for_multiplication = np.array(charges_for_multiplication)
    service_tax_rate_for_multiplication = np.array(service_tax_rate_for_multiplication)
    ...
    # mapping_index_numbers_where_guest_total_occured.to_excel(f"/home/caratred/Downloads/{file_naming}.xlsx",index=False)